 ...
 // public let dataPersistence = DataPersistence(filename: "schedules.plist")
  ...
  override func viewDidLoad() {
    ...
    override func viewWillDisappear(_ animated: Bool) { // every time you exit the vc
        // is a place where you can save the textfield text so that you don't have to press enter in order to save the text
  ...
  @IBAction func editButtonPressed(_ sender: UIBarButtonItem) {
    ...
    private func showCreateEventVC(_ event: Event? = nil) { // Event? = nil is a default parameter whoch means the user has the option to incorporate a parameter or not. It's not required.
        
        // this looks like a different way to segue? It's not. It presents the page but we're not segueing because we're not sending any information to this view controller
        // we need to use the storyboard to get an instance of the CreateEventController
        // all view controllers have a property called storyboard
        // Example: let loginStoryboard = UIStoryboard?
        guard let createEventController = storyboard?.instantiateViewController(identifier: "CreateEventController") as? CreateEventController else {
            fatalError("Could not downcast to CreateEventCntroller")
        }
        
       // let createEC = CreateEventController() - creates an empty view controller with out any outlets, etc, will crash if ypu are expecting your outlets to work if they exist in the storyboard
        
        // we did this opposed to just dragging because this allows us to create an instance of the view controller taht we can work with 
        
        // TODO:
         // that means if an event was passed intot his function
            createEventController.event = event
            
            
        
        // for updating an event we will inject("dependency injection") th esleected event
        // createEventController.event = event
