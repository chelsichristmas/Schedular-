...
class DataPersistence<T: Writable> {
  ...
  // Update
    // These two update functions can have the same name because they take in different arguments
    // Functions can share the same name so long as they take in different arguments
    // discradable result is a up to the discretion of the user. they can decide whether or not they want to use the result
    // discardable results silence the warning if the return value is not used
    
@discardableResult // silences the warnign if the return value is not used by the caller
